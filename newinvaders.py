...
objects = {'player': {'refpos': [0,216],
           ...
           'plshot': {'refpos': [0,0],
                      ...
                      'direction': (0,-4),
                      'cooldown': [0],
                      'sprite':[0*bm.shot, bm.shot, bm.shot_exploding, 0*bm.shot_exploding]
                      },
           'saucer': {'refpos': [0,40],
                      'active': 0,
                      'direction': (0,0),
                      'cooldown': 600,
                      'sprite':[0*bm.saucer, bm.saucer, bm.saucer_exploding, 0*bm.saucer_exploding]
                      },
           'aliens': {'refpos': [24,120],
                      'active': [1 for _ in range(55)],
                      'direction': (2,0),
                      'cooldown': [0],
                      'sprite':[0*bm.alien_exploding, bm.aliens, bm.alien_exploding, 0*bm.alien_exploding]
                     },
           'rolling': {'refpos': [0,0],
                      'active': 0,
                      'direction': (0,4),
                      'cooldown': 48,
                      'sprite':[0*bm.shot, bm.rolling, bm.alien_shot_exploding, 0*bm.alien_shot_exploding]
                     },
           'plunger': {'refpos': [0,0],
                      'active': 0,
                      'direction': (0,4),
                      'cooldown': 48,
                      'sprite':[0*bm.shot, bm.plunger, bm.alien_shot_exploding, 0*bm.alien_shot_exploding]
                     },
           'squigly': {'refpos': [0,0],
                      'active': 0,
                      'direction': (0,4),
                      'cooldown': 48,
                      'sprite':[0*bm.shot, bm.squigly, bm.alien_shot_exploding, 0*bm.alien_shot_exploding]
                     },
...
class CanvasClass(np.ndarray):
    ...
    def drawsprite(self, bitmap, position):
        ...
        self[position[1]:position[1]+bitmap.shape[0], position[0]:position[0]+bitmap.shape[1]] = bitmap
...
class GameObject():
    ...
    def move(self):
        ...
        if self.refpos[1]<=32 or self.refpos[1]+8 >=236:
            self.edgeflag = 1
        if (self.refpos[0]==0 and self.direction[0] < 0) or (self.refpos[0] == width-self.sprite[1].shape[1] and self.direction[0] > 0):
            self.edgeflag = 1
    ...
    def edgedetect(self, number=0):
        ...
        if (xcoord==0 and self.direction[0] < 0) or (xcoord == width-self.sprite[2].shape[1] and self.direction[0] > 0):
            return True
        return False
...
def main():
    ...
    alshot = [GameObject(objects["rolling"]), GameObject(objects["plunger"]), GameObject(objects["squigly"])]
    ...
    while running:
        ...
        if key[pygame.K_q]:
            pygame.quit()
        elif key[pygame.K_SPACE] and plshot.cooldown[0] == 0 and plshot.active[0] == 0:
        ...
        canvas.drawsprite(player.sprite[player.active[0]], player.refpos)

        # Move player's shot and draw shot
        plshot.cooldown[0] = np.maximum(plshot.cooldown[0]-1, 0)
        if plshot.active[0] == 1:
            plshot.move()
        if plshot.edgeflag:
            plshot.active[0] = 2
            plshot.edgeflag=0
        canvas.drawsprite(plshot.sprite[plshot.active[0]], plshot.refpos)
        if plshot.active[0] > 1:
            plshot.active[0] = (plshot.active[0]+1) % 4

        # Here goes alien shots
        shotnumber = tick%3
        alshot[shotnumber].cooldown = np.maximum(alshot[shotnumber].cooldown-1, 0)
        if shotnumber == 0:
            shotcolumn = canvas[48:192,player.refpos[0]+8]
        elif shotnumber == 1:
        ...
        if alshot[shotnumber].cooldown == 0 and sum(shotcolumn) != 0 and alshot[shotnumber].active == 0:
            alshot[shotnumber].active = 1
            for idx,he in enumerate(reversed(shotcolumn)):
                if he == 1:
                    break
            if shotnumber == 0:
        ...
        if alshot[shotnumber].edgeflag:
            alshot[shotnumber].active = 2
            alshot[shotnumber].edgeflag = 0
        if alshot[shotnumber].active == 1:
            canvas.drawsprite(alshot[shotnumber].sprite[alshot[shotnumber].active][tick %4], alshot[shotnumber].refpos)
        else:
            canvas.drawsprite(alshot[shotnumber].sprite[alshot[shotnumber].active], alshot[shotnumber].refpos)
        if alshot[shotnumber].active > 1:
        ...
        if saucer.active == 2: # Saucer was hit. Add score and increment status
                canvas.drawsprite(saucer.sprite[saucer.active], saucer.refpos)
                # Add score update
                saucer.active = (saucer.active+1) % 4
        if saucer.active == 3: # Saucer needs to disapper. Set status to inactive
                canvas.drawsprite(saucer.sprite[saucer.active], saucer.refpos)
                saucer.active = (saucer.active+1) % 4
